  ...
  return (
    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
          {footerLinks.map((footerLink) => (
              <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                ...
                <ul key={footerLink.title} className="list-none mt-4">
                  {footerLink.links.map((link, index) => (
                  <a key={link.name} href={link.link} target={link.target}>
                  <li key={link.name} className={`font-roboto font-normal text-[16px] leading-[24px] text-dimWhite hover:text-accent cursor-pointer ${ index !== footerLink.links.length -1 ? "mb-4": "mb-0"} `}>
                    {link.name}
                  </li>
                  </a>
                  ))}
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
        <p className="font-poppins font-normal text-center text-[14px] leading-[27px] text-white">2023 The Crown Pub Hawes Ltd. All Rights Reserved.</p>
